...
export default function CitySubjectCoordinatorsIndex({ coordinators, subjects }) {
    ...
    const submitCreate = (e) => {
        e.preventDefault();
        createForm.post(route('city.subject-coordinators.store'), { preserveScroll: true, onSuccess: () => setCreating(false) });
    };
    ...
    const submitEdit = (e) => {
        e.preventDefault();
        editForm.put(route('city.subject-coordinators.update', editC.id), { preserveScroll: true, onSuccess: () => setEditC(null) });
    };
    const remove = (c) => {
        if (confirm(`Удалить ответственного «${c.name}»?`)) {
            router.delete(route('city.subject-coordinators.destroy', c.id), { preserveScroll: true });
        }
    };

    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Ответственные по предметам</h2>}>
            <Head title="Ответственные по предметам" />

            <div className="py-8">
                <div className="mx-auto max-w-4xl space-y-4 px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between gap-4">
                        <p className="max-w-2xl text-sm text-gray-500">
                            Ответственный по предмету получает права муниципального координатора внутри Казани, но только
                            по назначенным предметам (состав, ввод результатов, председатели комиссий, протоколы).
                        </p>
                    ...
                    {coordinators.length === 0 ? (
                        <div className="rounded-lg bg-white p-10 text-center text-gray-400 shadow">Ответственные ещё не созданы.</div>
                    ) : (
            ...
            <Modal show={creating} onClose={() => setCreating(false)} maxWidth="lg">
                <form onSubmit={submitCreate} className="space-y-4 p-6">
                    <h3 className="font-semibold text-gray-800">Новый ответственный по предмету</h3>
            ...
            <Modal show={!!editC} onClose={() => setEditC(null)} maxWidth="lg">
                {editC && (
                    <form onSubmit={submitEdit} className="space-y-4 p-6">
                        <h3 className="font-semibold text-gray-800">Редактирование ответственного</h3>
